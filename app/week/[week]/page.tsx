  ...
  return (
    <main>
      <header className="topbar">
        ...
        <a className="mobile-history-link" href="/archive">歷史資料</a>
        <nav aria-label="週次頁導覽"><Link href="/">最新一期</Link><Link href="/archive">全部歷史</Link><a href="/feed.xml">Atom Feed</a></nav>
      </header>
      <section className="archive-hero week-hero">
        <div className="grid-noise" aria-hidden="true" />
        <div><p className="eyebrow">PERMANENT WEEKLY EDITION</p><h1>{week || "無效週次"}<span>必讀清單</span></h1><p>此網址固定保存該週入選內容，適合引用、分享與稽核追溯。</p><a className="secondary-button" href="/archive">← 返回歷史資料庫</a></div>
      ...
      <section className="library archive-library">
        {items.length ? <div className="reading-grid week-reading-grid">{items.map((reading) => <article className="reading-card" id={`reading-${reading.id}`} key={reading.id}>
          ...
          <h3>{reading.title}</h3><p className="card-subtitle">{reading.subtitle}</p><p className="card-summary">{reading.summary}</p>
          ...
          <div className="card-actions"><a href={reading.source} target="_blank" rel="noreferrer">原始來源 ↗</a>{reading.pdf && <a href={reading.pdf} target="_blank" rel="noreferrer">PDF ↓</a>}<a href={`#reading-${reading.id}`} aria-label={`複製 ${reading.title} 深連結`}>單篇連結 #</a></div>
        </article>)}</div> : <div className="empty-state"><b>WEEK NOT FOUND</b><p>找不到這個週次；請回到歷史資料庫選擇有效週次。</p></div>}
      </section>
      <footer><div className="brand footer-brand"><span className="brand-mark">AI</span><span><strong>Manufacturing AI Security</strong><small>VERIFIABLE WEEKLY ARCHIVE</small></span></div><p>固定週次網址 · 正體中文／臺灣慣用語</p><a href="/archive">歷史資料庫 →</a></footer>
    </main>
  );
}
