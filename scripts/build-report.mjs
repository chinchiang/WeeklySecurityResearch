...
export function generateReportForWeek(week) {
  ...
  const revision = editorial.revision ?? 1;
  const verifiedAt = editorial.verifiedAt ?? slug;
  const presentationNote = editorial.presentationNote ?? "";
  const reportSkipNote = editorial.reportSkipNote ?? "";

  const stats = {
    total: weekReadings.length,
    deep: weekReadings.filter((r) => r.decision === "深入審閱").length,
    selective: weekReadings.filter((r) => r.decision === "選讀").length,
  };

  const integration = weeklyReportIntegrations[week];

  const cards = weekReadings
    .map(
      (r) => `<article class="reading-card" id="reading-${r.id}">
  ...
  const integrationHtml = integration
    ? `
<section class="about"><h2>Claude 週報整合紀錄</h2><p>本期曾讀取：${esc(integration.title)}（私人歸檔不公開連結）</p><p>Drive 修改時間：${esc(integration.modifiedAt)}</p>${list(integration.adopted)}${list(integration.corrections)}<p>公開原始來源的核實紀錄見各篇；內部 WORK 責任、期限與完成證據不列入公開版。</p></section>`
    : "";

  const skippedHtml =
    editorial.skipped.length > 0
      ? `<ul>${editorial.skipped.map((s) => `<li><a href="${esc(s.source)}">${esc(s.title)}</a>：${esc(s.reason)}</li>`).join("")}</ul>`
      : "<p>本期無額外排除項目或初篩略過紀錄。</p>";
